...
⚠️ **Prețul de pe bonul de consum este valoarea de stoc a mișcării împărțită la cantitate** (`move.value`) — nu vine nici din comanda de vânzare, nici din vreo listă de prețuri. Fără un modul care să valorizeze mișcările interne, `move.value` rămâne 0, așa că pe un consum din magazie **Preț unitar și Valoare ies 0,00**, ca în captura de mai jos. Nu încercați să „reparați" asta completând o listă de prețuri: nu e citită aici. Cu `l10n_ro_stock_account` instalat (suita `l10n-romania-oca`) pe o companie cu planul de conturi RO, coloanele se populează automat — raportul citește deja `move.value`, fără nicio modificare aici. Fără el, documentul rămâne valabil ca dovadă a mișcării cantitative; pentru partea valorică se folosește nota contabilă de descărcare.
...
⚠️ La fel ca la bonul de consum, **coloanele valorice ies 0,00 pe transferurile interne** fără o sursă de valorizare — prețul se obține din valoarea mișcării împărțită la cantitate (`move.value`), iar Odoo 19 core nu valorizează mișcările care nu intră și nu ies din patrimoniu. Cu `l10n_ro_stock_account` instalat pe planul de conturi RO, valoarea reflectă costul din gestiunea sursă. Fără el, nota de transfer este un document cantitativ.
...
| `l10n_ro_stock_account` (suita `l10n-romania-oca`) | dacă e instalat și compania e pe planul de conturi RO, valorizează mișcările interne — bonul de consum și nota de transfer arată automat prețul și valoarea reale | complementar, fără dependență |
...
| Bonul de consum sau nota de transfer ies cu **0,00** la Preț unitar și Valoare | Ambele citesc valoarea de stoc a mișcării (`move.value`), dar niciun modul instalat nu valorizează mișcările interne — Odoo 19 core nu o face | Comportament normal fără sursă de cost, nu o limitare a acestui modul — completarea unei liste de prețuri nu ajută, nu e citită aici. Instalați `l10n_ro_stock_account` (suita `l10n-romania-oca`) pe o companie cu planul de conturi RO: raportul citește deja `move.value`, fără nicio modificare de cod aici |
