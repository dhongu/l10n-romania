...
- Fix: `_revolut_journal_exists` looked up the matching journal by IBAN alone
  ...
  journals sharing that IBAN.
- Fix: the same currency-blind lookup also existed in the base wizard's
  `_match_journal` (shared `account_statement_import_file` module, not
  touched here) - overridden locally for Revolut imports.
- Fix: the currency lookup in `_revolut_journal_exists` didn't set
  `active_test=False`, so a currency inactive in Settings (but still used
  by a journal) was treated as "not found" and its wallet dropped.
  Ported from 18.0 (ticket #9408, Agroamat).
