...
class RunDeclaration(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if "declaration_code" in self.env.context:
            declaration = self.env["l10n_ro.declaration"].search(
                [("code", "=", self.env.context["declaration_code"]), ("company_id", "=", self.env.user.company_id.id)]
            )
            if declaration:
    ...
    def prepare_value(self):
        vals = {"company_id": self.company_id.id, "date_from": self.date_from, "date_to": self.date_to}
        return vals
