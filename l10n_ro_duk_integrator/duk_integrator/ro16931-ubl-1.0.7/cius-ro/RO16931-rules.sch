...
<!--
   ...
   Schematron version 1.0.7 - Last update: 2022-10-18 (replace special and romanian characters in error messages; correction error messages for BR-RO-100, BR-RO-160, BR-RO-200)
...
<!-- Declaring global variables (in XSLT speak) -->
	...
	<rule context="/ubl:Invoice | /cn:CreditNote">
			...
			<assert test="count(cac:InvoiceLine) &lt;= 999"
				flag="fatal"
				id="BR-RO-A999" 
				>[BR-RO-A999]-O factura trebuie sa contina maximum 999 de aparitii ale grupului Linie a facturii (BG-25).
			...
			<assert test="string-length(normalize-space(cbc:ID)) &lt;= 30"
				flag="fatal" 
				id="BR-RO-L0301" 
				>[BR-RO-L030]-Numarul maxim permis de caractere pentru Numarul facturii (BT-1) este 30.
			...
			<assert test="string-length(normalize-space(cac:ContractDocumentReference/cbc:ID)) &lt;= 30"
				flag="fatal" 
				id="BR-RO-L0302" 
				>[BR-RO-L030]-Numarul maxim permis de caractere pentru Referinta contractului (BT-12) este 30.
			...
			<assert test="string-length(normalize-space(cac:OrderReference/cbc:ID)) &lt;= 30"
				flag="fatal" 
				id="BR-RO-L0303" 
				>[BR-RO-L030]-Numarul maxim permis de caractere pentru Referinta comenzii (BT-13) este 30.
			...
			<assert test="string-length(normalize-space(cac:OrderReference/cbc:SalesOrderID)) &lt;= 30"
				flag="fatal" 
				id="BR-RO-L0304" 
				>[BR-RO-L030]-Numarul maxim permis de caractere pentru Referinta dispozitiei de vanzare (BT-14) este 30.
			...
			<assert test="string-length(normalize-space(cac:ReceiptDocumentReference/cbc:ID)) &lt;= 30"
				flag="fatal" 
				id="BR-RO-L0305" 
				>[BR-RO-L030]-Numarul maxim permis de caractere pentru Referinta avizului de receptie (BT-15) este 30.
			...
			<assert test="string-length(normalize-space(cac:DespatchDocumentReference/cbc:ID)) &lt;= 30"
				flag="fatal" 
				id="BR-RO-L0306" 
				>[BR-RO-L030]-Numarul maxim permis de caractere pentru Referinta avizului de expeditie (BT-16) este 30.
			...
			<assert test="string-length(normalize-space(cac:OriginatorDocumentReference/cbc:ID)) &lt;= 30"
				flag="fatal" 
				id="BR-RO-L0307" 
				>[BR-RO-L030]-Numarul maxim permis de caractere pentru Referinta cererii de oferta sau a lotului (BT-17) este 30.
	...
	<rule context="//cac:Item/cac:AdditionalItemProperty"> 
		<assert test="count(.) &lt;= 50"
			flag="fatal"
			id="BR-RO-A052" 
			>[BR-RO-A052]-O factura trebuie sa contina maximum 50 de aparitii ale grupului Atributele articolului (BG-32).
	...
	<rule context="//cac:AdditionalDocumentReference">
		...
		<assert test="string-length(normalize-space(cbc:ID)) &lt;= 30"
			flag="fatal" 
			id="BR-RO-L0308" 
			>[BR-RO-L030]-Numarul maxim permis de caractere pentru Identificatorul obiectului facturat (BT-18) si Referinta documentului justificativ (BT-122) este 30.
	...
	<rule context="//cac:BillingReference">
		...
		<assert test="string-length(normalize-space(cac:InvoiceDocumentReference/cbc:ID)) &lt;= 30"
			flag="fatal" 
			id="BR-RO-L0309" 
			>[BR-RO-L030]-Numarul maxim permis de caractere pentru Referinta la o factura anterioara (BT-25) este 30.
