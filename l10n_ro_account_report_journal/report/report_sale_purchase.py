...
class SaleJournalReport(models.TransientModel):
    ...
    def _get_forreport_invoices_payments(self, data, journal_type):
        ...
        if journal_type == "sale":
            sale_purchase_domain = [("type", "in", ["out_invoice", "out_refund", "out_receipt"])]
        elif journal_type == "purchase":
            sale_purchase_domain = [("type", "in", ["in_invoice", "in_refund", "in_receipt"])]
        ...
        invoices_in_period_domain = (
            general_domain + sale_purchase_domain + [("invoice_date", ">=", date_from), ("invoice_date", "<=", date_to)]
        )
        ...
        older_unpaid_invoices = account_move_obj.search(
            ...
            + [
                ...
                ("invoice_payment_state", "in", ["in_payment", "not_paid"]),
            ]
        )
    ...
    def compute_report_lines(self, invoices, data, show_warnings=False, report_type_sale=True):
        ...
        sale_and_purchase_comun_columns = {
            ...
            "tva_neex": {
                "type": "int",
                "tags": ["-09_1 - TVA", "+09_1 - TVA", "-10_1 - TVA", "+10_1 - TVA", "-11_1 - TVA", "+11_1 - TVA",],
            },
        ...
        for k, v in sale_and_purchase_comun_columns.items():
            for tag in v["tags"]:
                if tag in all_known_tags.keys():
                    ...
                    warn = f"tag='{tag}' exist in column={k} but " f"also in column='{all_known_tags[tag]}'"
                    ...
                    if not line.tax_exigible:  # VAT on payment
                        ...
                        if unknown_line:
                            vals["warnings"] += (
                                ...
                                f"credit={line.credit} TAGS{[x.name for x in line.tag_ids]};"
                            )
