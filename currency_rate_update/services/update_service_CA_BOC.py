    ...
    def get_updated_currency(self, currency_array, main_currency, max_delta_days):
        ...
        for curr in currency_array:
            ...
            if dom.bozo and dom.status != 404:
                _logger.error(
                    "Bank of Canada - service is down - try again\
                    later..."
                )
            ...
            if dom.status != 200:
                _logger.error(
                    "Exchange data for %s is not reported by Bank\
                    of Canada."
                    % curr
                )
                raise except_orm(
                    _("Error !"), _("Exchange data for %s is not " "reported by Bank of Canada." % str(curr))
                )

            _logger.debug("BOC sent a valid RSS file for: " + curr)

            # check for valid exchange data
            if (dom.entries[0].cb_basecurrency == main_currency) and (dom.entries[0].cb_targetcurrency == curr):
                rate = dom.entries[0].cb_exchangerate.split("\n", 1)[0]
                rate_date_datetime = parser.parse(dom.entries[0].updated).astimezone(pytz.utc).replace(tzinfo=None)
                ...
                raise except_orm(_("Error !"), _("Exchange data format error for " "Bank of Canada - %s !" % str(curr)))
