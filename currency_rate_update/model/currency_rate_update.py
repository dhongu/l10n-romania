...
class CurrencyRateUpdateService(models.Model):
    ...
    currency_list = fields.Many2many(
        "res.currency", "res_currency_update_avail_rel", "service_id", "currency_id", string="Currencies available"
    )
    ...
    company_id = fields.Many2one(
        ...
        "Company",
        required=True,
        default=lambda self: self.env["res.company"]._company_default_get("currency.rate.update.service"),
    )
    ...
    def refresh_currency(self):
        ...
        for srv in self:
            _logger.info(
                "Starting to refresh currencies with service %s (company: %s)", srv.service, srv.company_id.name
            )
            ...
            if company.auto_currency_up:
                ...
                try:
                    ...
                    for curr in srv.currency_to_update:
                        ...
                        rates = rate_obj.search(
                            [("currency_id", "=", curr.id), ("company_id", "=", company.id), ("name", "=", rate_name)]
                        )
                        if not rates:
                            vals = {
                                "currency_id": curr.id,
                ...
                if self._context.get("cron"):
                    midnight = time(0, 0)
                    next_run = (
                        datetime.combine(fields.Date.from_string(srv.next_run), midnight)
                        + _intervalTypes[str(srv.interval_type)](srv.interval_number)
                    ).date()
