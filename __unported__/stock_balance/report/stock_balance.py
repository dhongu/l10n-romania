...
class stock_balance(models.Model):
    ...
    qty_in = fields.Float('Qty In', digits=dp.get_precision('Product Unit of Measure'), readonly=True)
    amount_in = fields.Float('Amount In', digits=dp.get_precision('Account'), readonly=True)
    qty_out = fields.Float('Qty Out', digits=dp.get_precision('Product Unit of Measure'), readonly=True)
