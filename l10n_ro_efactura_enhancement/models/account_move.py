...
class AccountMove(models.Model):
    ...
    def _l10n_ro_spv_send_cron_report(self, company, stats):
        ...
        rows = [
            (self.env._("Trimise cu succes în SPV"), len(stats["sent_ok"]), _names(stats["sent_ok"])),
        ...
        body = (
            f"<p>{self.env._('Raport rulare cron trimitere e-Factura în SPV')} — "
            f"<b>{company.name}</b><br/>"
            f"{self.env._('Data rulării')}: {fields.Datetime.to_string(now)}</p>"
            f"<p>{self.env._('Total facturi procesate la această rulare')}: "
            f"<b>{len(stats['attempted'])}</b></p>"
            "<table style='border-collapse:collapse;font-size:13px;'>"
            f"<tr style='background:#f5f5f5;'>"
            f"<th style='padding:4px 8px;border:1px solid #ddd;text-align:left;'>{self.env._('Categorie')}</th>"
            f"<th style='padding:4px 8px;border:1px solid #ddd;'>{self.env._('Nr.')}</th>"
            f"<th style='padding:4px 8px;border:1px solid #ddd;text-align:left;'>{self.env._('Facturi')}</th>"
            f"</tr>{body_rows}</table>"
        )

        mail_values = {
            "subject": self.env._("[%s] Raport e-Factura SPV", company.name),
            "body_html": body,
            "email_to": recipients,
