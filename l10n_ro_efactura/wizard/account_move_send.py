...
class AccountMoveSend(models.TransientModel):
    ...
    l10n_ro_edi_send_readonly = fields.Boolean(
    ...
    l10n_ro_edi_warnings = fields.Char(
    ...
    def _compute_l10n_ro_edi_send_enable(self):
        ...
        for wizard in self:
            wizard.l10n_ro_edi_send_enable = any(
                (move._need_ubl_cii_xml() or move.ubl_cii_xml_id)
                and move.country_code == "RO"
                and move.l10n_ro_edi_state in (False, "invoice_sending")
    ...
    def _compute_l10n_ro_edi_warnings(self):
        ...
        for wizard in self:
            waiting_moves = wizard.move_ids.filtered(
                lambda m: m.l10n_ro_edi_state == "invoice_sending"
            )
            wizard.l10n_ro_edi_warnings = (
                _(
    ...
    def _compute_checkbox_ubl_cii_xml(self):
        ...
        for wizard in self:
            if (
                wizard.l10n_ro_edi_send_checkbox
                and wizard.enable_ubl_cii_xml
                and not wizard.checkbox_ubl_cii_xml
            ):
        ...
        for invoice, invoice_data in invoices_data.items():
            if invoice_data.get("l10n_ro_edi_send") and not invoice.l10n_ro_edi_state:
                if invoice_data.get("ubl_cii_xml_attachment_values"):
                    ...
                    xml_data, build_errors = self.env[
                        "account.edi.xml.ubl_ro"
                    ]._export_invoice(invoice)
                    if build_errors:
                        invoice_data["error"] = {
                            "error_title": _(
                                "Error when rebuilding the CIUS-RO E-Factura XML"
                            ),
                            "errors": build_errors,
                        }
                        continue
