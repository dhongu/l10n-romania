...
class ResCompany(models.Model):
    ...
    l10n_ro_share_capital = fields.Float(string="Romania - Share Capital", digits="Account", default=200)
    l10n_ro_caen_code = fields.Char(related="partner_id.l10n_ro_caen_code", readonly=False)
    l10n_ro_account_serv_sale_tax_id = fields.Many2one("account.tax", string="Romania - Default Services Sale Tax")
    ...
    l10n_ro_property_stock_price_difference_product_id = fields.Many2one(
        ...
        domain="[('type', '=', 'service')]",
        help="This product will be used to create the landed cost" "for the price difference between picking and bill",
    )
    l10n_ro_property_customs_duty_product_id = fields.Many2one(
        "product.product",
        string="Romania - Customs Duty Landed Cost Product",
        domain="[('type', '=', 'service')]",
        help="This product will be used in create the DVI landed cost" "for the duty tax",
    )
    l10n_ro_property_customs_commission_product_id = fields.Many2one(
        "product.product",
        string="Romania - Customs Commission Landed Cost Product",
        domain="[('type', '=', 'service')]",
        help="This product will be used in create the DVI landed cost" "for the duty commissions.",
    )
