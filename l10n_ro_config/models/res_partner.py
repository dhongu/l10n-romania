    ...
    def _split_vat(self, vat):
        ...
        if vat and vat.isdigit():
            ...
            if partner and partner.country_id and partner.country_id.code:
                vat_country = self._l10n_ro_map_vat_country_code(partner.country_id.code.upper()).lower()
        else:
        ...
        if self.is_l10n_ro_record:
            if not self.env.context.get("skip_ro_vat_change"):
                if self.vat and self.vat.isdigit() and self.l10n_ro_vat_subjected:
                    vat_country = self._l10n_ro_map_vat_country_code(self.country_id.code.upper())
                    self.vat = vat_country + self.vat
                elif self.vat and not self.vat.isdigit() and not self.l10n_ro_vat_subjected:
