...
class AccountEdiFormat(models.Model):
    """This edi_format is "abstract" meaning that it provides an
    additional layer for similar edi_format (formats deriving from EN16931)
     that share some functionalities but needs to be extended to be used.
    """
    ...
    def _get_ubl_values(self, invoice):
        """Get the necessary values to generate the XML. These values will
         be used in the qweb template when
        rendering. Needed values differ depending on the implementation
         of the UBL, as (sub)template can be overriden
        or called dynamically.
        :returns:   a dictionary with the value used in the template
        has key and the value as value.
        """
    ...
    def _get_bis3_values(self, invoice):
        ...
        for line_vals in values["invoice_line_vals_list"]:
            if len(values["tax_details"]["invoice_line_tax_details"][line_vals["line"]]["tax_details"]) > 1:
                raise UserError(_("Multiple vat percentage not supported on the same invoice line"))
        ...
        for line_vals in values["invoice_line_vals_list"]:
            ...
            for tax_detail in tax_details_no_tax_closing["invoice_line_tax_details"][line_vals["line"]][
                "tax_details"
            ].values():
                line_vals["price_subtotal_with_no_tax_closing"] += tax_detail["tax_amount_currency"]

        values["total_untaxed_amount"] = sum(
            x["price_subtotal_with_no_tax_closing"] for x in values["invoice_line_vals_list"]
        )
    ...
    def _bis3_get_extra_partner_domains(self, tree):
        """Returns an additional domain to find the partner of the invoice
           based on specific implementation of BIS3.
        TO OVERRIDE
    ...
    def _decode_bis3(self, tree, invoice):
        ...
        with Form(
            invoice.with_context(default_move_type=move_type, default_journal_id=default_journal.id)
        ) as invoice_form:
            # Reference
            ...
            for eline in tree.findall(".//{*}InvoiceLine"):
                with invoice_form.invoice_line_ids.new() as invoice_line_form:
                    ...
                    invoice_line_form.price_unit = (
                        price_unit or line_extension_amount / invoice_line_form.quantity or 0.0
                    )
