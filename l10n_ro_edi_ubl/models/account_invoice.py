...
class AccountMove(models.Model):
    _name = "account.move"
    _inherit = ["account.move", "base.ro.ubl"]
    ...
    def _ubl_add_attachments(self, parent_node, ns, version="2.1"):
        ...
        if embed_pdf_in_ubl_xml_invoice and not self.env.context.get("no_embedded_pdf"):
    ...
    def _ubl_add_invoice_line(self, parent_node, iline, line_number, ns, version="2.1"):
        ...
        if iline.product_uom_id and iline.product_uom_id.name:  # iline.uom_id.unece_code:
            uom_unece_code = iline.product_uom_id.name[:3]  # iline.uom_id.unece_code
            if uom_unece_code.lower() in ("uni", "buc"):
        ...
        self._ubl_add_item(iline.name, iline.product_id, line_root, ns, type_="sale", version=version)
    ...
    def get_ubl_filename(self, version="2.1"):
        """This method is designed to be inherited"""
        return "UBL-Invoice-{}-{}.xml".format(version, self.name)
