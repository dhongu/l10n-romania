...
class BaseUbl(models.AbstractModel):
    ...
    def _ubl_add_address(self, partner, node_name, parent_node, ns, version="2.1"):
        ...
        if partner.state_id:
            ...
            state.text = "{}-{}".format(partner.country_id.code, partner.state_id.code)
    ...
    @api.model
    def _ubl_add_party_tax_scheme(self, commercial_partner, parent_node, ns, version="2.1"):
    ...
    @api.model
    def _ubl_add_supplier_party(self, partner, company, node_name, parent_node, ns, version="2.1"):
        ...
        partner_ref = self._ubl_get_customer_assigned_id(partner)
        if partner_ref:
            supplier_ref = etree.SubElement(supplier_party_root, ns["cbc"] + "CustomerAssignedAccountID")
            supplier_ref.text = partner_ref
        self._ubl_add_party(partner, company, "Party", supplier_party_root, ns, version=version)
    ...
    def _ubl_add_delivery(self, delivery_partner, parent_node, ns, version="2.1"):
        ...
        self._ubl_add_address(delivery_partner, "Address", delivery_location, ns, version=version)
        # self._ubl_add_party(delivery_partner, False, "DeliveryParty", delivery, ns, version=version)
    ...
    def _ubl_add_line_item(
        ...
        quantity_node = etree.SubElement(line_item, ns["cbc"] + "Quantity", unitCode=uom.unece_code)
        quantity_node.text = str(quantity)
        if currency and price_subtotal:
            line_amount = etree.SubElement(line_item, ns["cbc"] + "LineExtensionAmount", currencyID=currency.name)
            ...
            if not float_is_zero(quantity, precision_digits=qty_precision):
                price_unit = float_round(price_subtotal / float(quantity), precision_digits=price_precision)
            price = etree.SubElement(line_item, ns["cac"] + "Price")
            price_amount = etree.SubElement(price, ns["cbc"] + "PriceAmount", currencyID=currency.name)
    ...
    def _ubl_add_item(
        ...
        if product:
            ...
            if not product_name:
                ...
                product_name = variant and "{} ({})".format(product.name, variant) or product.name
        ...
        if customer_code:
            buyer_identification = etree.SubElement(item, ns["cac"] + "BuyersItemIdentification")
            buyer_identification_id = etree.SubElement(buyer_identification, ns["cbc"] + "ID")
    ...
    def _ubl_get_version(self, xml_root, root_name, ns):
        version_xpath = xml_root.xpath("/%s/cbc:UBLVersionID" % root_name, namespaces=ns)
    ...
    def _ubl_check_xml_schema(self, xml_string, document, version="2.1"):
        ...
        # xsd_file = "l10n_ro_edi_ubl/data/xsd-{}/UBL-{}-{}.xsd".format(version, document, version)
    ...
    def _ubl_add_xml_in_pdf_buffer(self, xml_string, xml_filename, buffer):
        ...
        writer._root_object.update({NameObject("/PageMode"): NameObject("/UseAttachments")})
        new_buffer = BytesIO()
    ...
    def embed_xml_in_pdf(self, xml_string, xml_filename, pdf_content=None, pdf_file=None):
        ...
        if pdf_file:
            with open(pdf_file, "rb") as f:
                pdf_content = f.read()
        updated_pdf_content = self._embed_ubl_xml_in_pdf_content(xml_string, xml_filename, pdf_content)
        if pdf_file:
    ...
    def ubl_parse_party(self, party_node, ns):
        ...
        contact_email_xpath = party_node.xpath("cac:Contact/cbc:ElectronicMail", namespaces=ns)
        contact_phone_xpath = party_node.xpath("cac:Contact/cbc:Telephone", namespaces=ns)
        partner_dict = {
        ...
        for id_node in id_nodes:
            id_numbers.append({"value": id_node.text, "schemeID": id_node.attrib.get("schemeID")})
    ...
    def ubl_parse_address(self, address_node, ns):
        country_code_xpath = address_node.xpath("cac:Country/cbc:IdentificationCode", namespaces=ns)
    ...
    def ubl_parse_delivery(self, delivery_node, ns):
        ...
        if party_xpath:
            ...
            partner_dict = {}
        postal_xpath = delivery_node.xpath("cac:DeliveryParty/cac:PostalAddress", namespaces=ns)
        if not postal_xpath:
            delivery_address_xpath = delivery_node.xpath("cac:DeliveryLocation/cac:Address", namespaces=ns)
            ...
            if delivery_address_xpath:
                partner_dict.update(self.ubl_parse_address(delivery_address_xpath[0], ns))
        return partner_dict
        ...
        try:
            ...
            for filename, xml_file_dict_obj in xmlfiles.items():
                try:
                    ...
                    logger.debug("A valid XML file %s has been found in the PDF file", filename)
