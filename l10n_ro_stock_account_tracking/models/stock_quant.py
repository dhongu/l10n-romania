...
class StockQuant(models.Model):
    ...
    def _compute_value(self):
        ro_quants = self.filtered(
            lambda quant: quant.company_id.l10n_ro_accounting and quant.location_id.usage == "internal"
        )
        super(StockQuant, self - ro_quants)._compute_value()
        quants_without_lot_allocation = ro_quants.filtered(
            lambda q: not q.company_id.l10n_ro_stock_account_svl_lot_allocation and q.lot_id
        )
        ...
        for product in products:
            for location in locations:
                quants = quants_without_lot_allocation.filtered(
                    lambda quant: quant.location_id == location and quant.product_id == product
                ).sorted("in_date")
                svls = (
                ...
                sq_lots = quants.filtered(lambda quant: quant.lot_id in svl_lots)
                svl_with_lot_in_sq_lot = svls.filtered(lambda svl: svl.l10n_ro_lot_ids in sq_lots.mapped("lot_id"))
                for svl in svl_with_lot_in_sq_lot:
                    for sq in sq_lots:
                        if svl.remaining_qty == sq.quantity and sq.lot_id in svl.l10n_ro_lot_ids:
                            quants -= sq
