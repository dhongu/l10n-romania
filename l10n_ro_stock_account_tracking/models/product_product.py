...
class ProductProduct(models.Model):
    ...
    def _compute_value_svl(self):
        ...
        if l10n_ro_records:
            ...
            use_svl_lot_config = company.l10n_ro_stock_account_svl_lot_allocation or self.env.context.get(
                "force_svl_lot_config", False
            )
            domain_ctx = []
            ...
            for group in groups:
                ...
                product.value_svl = self.env.company.currency_id.round(group["remaining_value"])
    ...
    def _prepare_out_svl_vals(self, quantity, company):
        ...
        if self.cost_method in ("average", "fifo"):
            ...
            for fifo_vals in fifo_vals_list:
                ...
                if self.cost_method == "average" and not self.env.context.get("origin_return_candidates"):
                    vals["value"] = currency.round(vals["quantity"] * self.standard_price)
                ...
                if self.cost_method == "fifo" or (
                    self.cost_method == "average" and self.env.context.get("origin_return_candidates")
                ):
    ...
    def _run_fifo(self, quantity, company):
        ...
        self.ensure_one()
        domain = self._l10n_ro_prepare_domain_fifo(company, [("product_id", "=", self.id)]) + [
            ("remaining_qty", ">", 0),
            ("company_id", "=", company.id),
        ]
    ...
    def _run_fifo_vacuum(self, company=None):
        ...
        for location in svls_to_vacuum.mapped("l10n_ro_location_id"):
            svl_loc_to_vaccum = svls_to_vacuum.filtered(lambda r: r.l10n_ro_location_id == location)
            loc_candidates = all_candidates.filtered(lambda r: r.l10n_ro_location_dest_id == location)
            if not loc_candidates:
                parent_locations = self.env["stock.location"].search(
                    [("parent_path", "=like", location.parent_path + "%")]
                )
                loc_candidates = all_candidates.filtered(lambda r: r.l10n_ro_location_dest_id in parent_locations)
    ...
    def _svl_replenish_stock(self, description, products_orig_quantity_svl):
        ...
        if self - l10n_ro_records:
            refill_stock_svl_list = super(ProductProduct, self - l10n_ro_records)._svl_replenish_stock(
