...
class StockMove(models.Model):
        ...
        if l10n_ro_records and self.env.context.get("standard"):
            ...
            for move in l10n_ro_records:
                ...
                if not valued_move_lines and forced_quantity:
                    ...
                    svl_vals = move.product_id._prepare_in_svl_vals(forced_quantity, unit_cost)
                ...
                for valued_move_line in valued_move_lines:
                    ...
                    if move.origin_returned_move_id and move.origin_returned_move_id.sudo().stock_valuation_layer_ids:
                        ...
                        origin_svls = move._l10n_ro_filter_svl_on_move_line(origin_domain)
                        if origin_svls:
            ...
            for move in self:
                ...
                for valued_move_line in valued_move_lines:
                    ...
                    for svl_vals in svl_vals_list:
                        ...
                        if forced_quantity:
                            svl_vals["description"] = (
                                "Correction of %s (modification of past move)" % valued_move_line.picking_id.name
    ...
    def _create_delivery_return_svl(self, forced_quantity=None):
        ...
        for move in l10n_ro_records:
            ...
            out_svls = []
            origin_svls = move.origin_returned_move_id.sudo().stock_valuation_layer_ids.sorted(
    ...
    def _l10n_ro_create_track_svl(self, value_list, **kwargs):
        ...
        for _index, value in enumerate(value_list):
            svl_value = sudo_svl._l10n_ro_pre_process_value(value)  # eliminam datele de tracking, filtram valorile SVL
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        internal_transfer_moves = moves.filtered("is_l10n_ro_record").filtered(lambda m: m._is_internal_transfer())
