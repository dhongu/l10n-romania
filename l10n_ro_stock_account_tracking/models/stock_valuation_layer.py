...
class StockValuationLayer(models.Model):
    ...
    def _compute_l10n_ro_svl_locations_lot(self):
        for svl in self:
            record = svl.l10n_ro_stock_move_line_id if svl.l10n_ro_stock_move_line_id else svl.stock_move_id
            svl.l10n_ro_location_id = record.location_id
            svl.l10n_ro_location_dest_id = record.location_dest_id
            svl.l10n_ro_lot_ids = record.lot_id if "lot_id" in record._fields else record.lot_ids

    def _compute_l10n_ro_svl_tracking(self):
        for s in self:
            s.l10n_ro_svl_dest_ids = [(6, 0, s.l10n_ro_svl_track_dest_ids.mapped("svl_dest_id").ids)]
            s.l10n_ro_svl_src_ids = [(6, 0, s.l10n_ro_svl_track_src_ids.mapped("svl_src_id").ids)]
    ...
    def _l10n_ro_create_tracking(self, source_svl_qty):
        ...
        svl_tracking_ids = self.env["l10n.ro.stock.valuation.layer.tracking"].create(tracking_values)
