...
class TestStockQuantValue(RoTestStockCommon):
    def test_stock_quant_value(self):
        ...
        for series in ["Series 1", "Series 2", "Series 3", "Series 4", "Series 5"]:
            stock_quant_qty = self.get_stock_quant_qty(series, product_serial_number, stock_location)
            stock_valuation_layer_qty = self.get_stock_valuation_layer_qty(series, product_serial_number)
            self.assertEqual(
                stock_quant_qty,
    ...
    def create_stock_move_in(self, series, date, qty, price_unit, product, stock_location, supplier_location):
    ...
    def create_stock_move_out(self, series, date, qty, product, stock_location, supplier_location):
        lot = self.env["stock.lot"].search([("name", "=", series), ("product_id", "=", product.id)])
        stock_move = self.env["stock.move"].create(
            {
