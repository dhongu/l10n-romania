...
class AccountInvoice(models.Model):
    ...
    def action_invoice_open(self):
        ...
        if to_open_invoices.filtered(
            ...
            and float_compare(inv.amount_total, 0.0, precision_rounding=inv.currency_id.rounding) == -1
        ):
            raise UserError(
                _(
        ...
        for payment in self.payment_move_line_ids:
            ...
            if payment.matched_debit_ids:
                payment_currency_id = (
                    ...
                    if all(
                        [p.currency_id == payment.matched_debit_ids[0].currency_id for p in payment.matched_debit_ids]
                    )
                    else False
                )
            if payment.matched_credit_ids:
                payment_currency_id = (
                    ...
                    if all(
                        [p.currency_id == payment.matched_credit_ids[0].currency_id for p in payment.matched_credit_ids]
                    )
                    else False
                )
            ...
            if payment_currency_id and payment_currency_id == self.currency_id:
                ...
                amount_to_show = payment.company_id.currency_id.with_context(date=self.date).compute(
